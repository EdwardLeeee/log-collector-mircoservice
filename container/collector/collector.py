...
def generate_api_token(ip_address):
    ...
    hashed_token = hashlib.sha256((api_token + ip_address).encode()).hexdigest()
    expiration_date = datetime.now() + timedelta(hours=TOKEN_EXPIRATION_HOURS)# 到期時間
    return hashed_token, expiration_date
...
def verify_and_generate_token():
    ...
    if client_ip in whitelist_ips:
        hashed_token, expiration_date = generate_api_token(client_ip)# 拿到token 和到期時間
        API_TOKENS[hashed_token] = expiration_date
        #print("API_TOKENS:", API_TOKENS)
        # 回傳API Key給client
        return jsonify({
            "collector-api-token": hashed_token,
            "expire-time": expiration_date.isoformat()
        }),200
    else:
        return jsonify({"mesage": f"IP not in whitelist , your ip is {client_ip}"}), 403

def validate_api_token(f):# f 是原函數
    def decorator(*args, **kwargs):#*args 和 **kwargs 允許裝飾器接收任意數量的位置參數和關鍵字參數
        api_token = request.headers.get('collector-api-token')
        expiration_date = API_TOKENS.get(api_token)# = API_TOKENS[api_token] or None
        if not api_token:# 沒有那把 token
            return jsonify({"message": "Unauthorized access (Wrong token or collector restarted). Please delete old token and acquire new token."}), 401
        if datetime.now() > expiration_date:
            return jsonify({"message": "API token expired. Please delete old token and acquire new token."}), 401
...
def process_raw_log():
    try:
        ...
        # 傳送 log 資料至最終儲存端點
        # response = requests.post('http://172.20.10.3:5000/save-log', json=log_data)
    ...
    except MissingDataError as e:# missing_fields 用到
        print(f"Error: {e}")
        return jsonify({"message": str(e)}), 400

    except InvalidLogLevelError as e:# check_error 用到
        print(f"Error: {e}")
