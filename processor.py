...
class LogHandler(FileSystemEventHandler):  # 繼承FileSystemEventHandler
    def __init__(self, config, host_info, offsets, save_offsets_func, collector_url,api_token_data):
        ...
        self.api_token_data = api_token_data
        # 初始化,更新一遍所有log狀態
    ...
    def send_to_collector(self, log_data):
        try:
            headers={"collector-api-token": self.api_token_data['collector-api-token']}
            response = requests.post(f'{self.collector_url}/send-log', json=log_data , headers = headers)
            if response.status_code == 401:# 如果401，重新请求新的API Key
                print(f"API token error: {response.status_code}, {response.json().get('message','N/A')}")
                api_manager = APIManager(self.collector_url)
                self.api_token_data = api_manager.load_api_token()

                if self.api_token_data:
                    headers={"collector-api-token": self.api_token_data['collector-api-token']}
                    response = requests.post(f'{self.collector_url}/send-log', json=log_data,headers=headers)
                else:
                    print("Failed to load API token.")
                    sys.exit(1)  # 退出程式 避免没有API token
